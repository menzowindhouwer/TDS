/* Copied from http://www.onjava.com/pub/a/onjava/excerpt/java_xslt_ch5/index.html?page=7 */
...
public class CSVXMLReader extends AbstractXMLReader {
  ...
  // constructor
  public CSVXMLReader(String delimeter) {
      super();
      if ((delimeter!=null) && (delimeter.length()==1))
          COMMA = delimeter.charAt(0);
  }
  ...
  private int locateFirstDelimiter(String curLine) {
    if (curLine.startsWith("\"")) {
      ...
      for (int i=1; i<numChars; i++) {
        ...
        if (curChar == '"') {
          inQuote = !inQuote;
        } else if (curChar == COMMA && !inQuote) {
          return i;
        }
      }
      return -1;
    } else {
      return curLine.indexOf(COMMA);
    }
  }
